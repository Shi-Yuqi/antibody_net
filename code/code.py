...
# 加载存档
model = RoFormerModel.from_pretrained("D:/code/anti_net/pre-train/antiberta2-cssp")
tokenizer = RoFormerTokenizer.from_pretrained("D:/code/anti_net/pre-train/antiberta2-cssp")
...
def train_model(model2, criterion, optimizer, train_Ab_list, train_Ag_list, train_labels):
    model2.train(True) # 设置为训练模式
    optimizer.zero_grad() 
    output = model2(train_Ab_list,train_Ag_list) 
    output = torch.tensor(output, dtype=torch.float,requires_grad=True)
    loss = criterion(output, train_labels)  
    loss.backward()  # 反向传播  
    torch.nn.utils.clip_grad_norm_(model2.parameters(), max_norm=1)  # 梯度裁剪
    optimizer.step()  # 更新参数  
    torch.save(model2.state_dict(), 'D:\code\MyNet\model_2\code/model_parameters.pth') 
    return loss
def val_model(model2, criterion, optimizer, val_Ab_list, val_Ag_list, val_labels):
    model2.eval()  # 评估模式
    with torch.no_grad():
        val_output = model2(val_Ab_list,val_Ag_list)
        val_output = torch.tensor(val_output, dtype=torch.float,requires_grad=True)
        val_loss = criterion(val_output, val_labels)
        rmse = val_loss.item() ** (1/2)
        return rmse
...
best_rmse = float('inf')  # 初始化最佳的验证集RMSE为正无穷  
best_model_state = None  # 初始化最佳模型状态为空  
  
for epoch in range(100):  
    loss = train_model(model2, criterion, optimizer, train_Ab_list, train_Ag_list, train_labels)  
    rmse = val_model(model2, criterion, optimizer, val_Ab_list, val_Ag_list, val_labels)  
      
    if rmse < best_rmse:  # 如果当前模型的性能更好  
        best_rmse = rmse  # 更新最佳的验证集RMSE  
        best_model_state = model2.state_dict()  # 保存当前模型的状态字典  
  
    if epoch % 10 == 0:  
        print(f'Epoch {epoch+1}, Loss: {loss.item()}, Val RMSE: {rmse}')  
# 在整个训练过程结束后，加载最佳模型的状态进行最终评估  
model2.load_state_dict(best_model_state)  
final_rmse = val_model(model2, criterion, optimizer, val_Ab_list, val_Ag_list, val_labels)  
